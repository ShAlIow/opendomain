...
check_env() {
    ...
    if [ ! -f ".env" ]; then
        ...
        echo ""
        echo "必须配置的变量："
        echo "  - DB_PASSWORD        (OpenDomain 数据库密码)"
        echo "  - JWT_SECRET         (JWT 密钥)"
        echo "  - POWERDNS_API_KEY   (PowerDNS API 密钥，app 与 pdns 共用)"
        echo "  - PDNS_DB_PASSWORD   (PowerDNS MariaDB 密码)"
        echo "  - PDNS_ADMIN_SECRET  (PowerDNS Admin 面板 Secret Key)"
...
show_status() {
    ...
    echo ""
    echo "服务访问地址："
    echo "  - API 后端:          http://localhost:${PORT:-8000}"
    echo "  - 健康检查:          http://localhost:${PORT:-8000}/health"
    echo "  - PowerDNS API:      http://localhost:${PDNS_API_PORT:-8081}"
    echo "  - PowerDNS Admin:    http://localhost:${PDNS_ADMIN_PORT:-9191}"
